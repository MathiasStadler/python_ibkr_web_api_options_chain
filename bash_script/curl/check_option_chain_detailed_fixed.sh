...
# Statistik Variablen (ohne declare für Kompatibilität)
TOTAL_TESTS=0
VALID_OPTIONS=0
INVALID_OPTIONS=0
MARKET_DATA_SUCCESS=0
MARKET_DATA_FAILED=0
VALID_CONIDS=""
INVALID_CONIDS=""
...
# 4. CONIDs in Batches testen (ohne Arrays für Kompatibilität)
CONID_LIST=$(echo "$CONIDS" | tr '\n' ' ')
CONID_COUNT=$TOTAL_CONIDS
TEST_LIMIT=$((CONID_COUNT < MAX_TESTS ? CONID_COUNT : MAX_TESTS))

echo -e "${YELLOW}Fortschritt:${NC}"

# Position im CONID List
POS=1
PROCESSED_CONIDS=""

while [ $TOTAL_TESTS -lt $TEST_LIMIT ] && [ $VALID_OPTIONS -lt $MIN_VALID_OPTIONS ]; do
    # Batch erstellen
    BATCH_CONIDS=""
    BATCH_COUNT=0
    
    for ((j=0; j<BATCH_SIZE && TOTAL_TESTS < TEST_LIMIT && VALID_OPTIONS < MIN_VALID_OPTIONS; j++)); do
        # Nächste CONID aus der Liste holen
        CONID=$(echo "$CONID_LIST" | cut -d' ' -f$POS)
        
        if [ -n "$CONID" ] && [ "$CONID" != "null" ]; then
            BATCH_CONIDS="${BATCH_CONIDS}${CONID},"
            BATCH_COUNT=$((BATCH_COUNT + 1))
            TOTAL_TESTS=$((TOTAL_TESTS + 1))
            PROCESSED_CONIDS="$PROCESSED_CONIDS $CONID"
            POS=$((POS + 1))
        else
            break
        fi
    ...
    # Progress-Bar
    PERCENTAGE=$(( (TOTAL_TESTS * 100) / TEST_LIMIT ))
    BAR_WIDTH=50
    ...
    if [ "$MARKET_HTTP_STATUS" = "200" ]; then
        ...
        for CONID in $BATCH_CONIDS; do
            ...
            if [ -n "$CONID_DATA" ] && [ "$CONID_DATA" != "null" ]; then
                ...
                if echo "$BID" | grep -qE '^[0-9.]+$' && echo "$ASK" | grep -qE '^[0-9.]+$'; then
                    HAS_BID_ASK=true
                fi
                
                if echo "$VOLUME" | grep -qE '^[0-9.]+$'; then
                    VOL_INT=$(echo "$VOLUME" | cut -d. -f1)
                    if [ "$VOL_INT" -ge 0 ] 2>/dev/null; then
                        HAS_VOLUME=true
                    fi
                fi
                
                if echo "$OI" | grep -qE '^[0-9.]+$'; then
                    OI_INT=$(echo "$OI" | cut -d. -f1)
                    if [ "$OI_INT" -ge 0 ] 2>/dev/null; then
                        HAS_OI=true
                    fi
                fi
                ...
                if [ "$HAS_BID_ASK" = true ]; then
                    ...
                    VALID_CONIDS="$VALID_CONIDS $CONID"
                    ...
                    VALID_COUNT=$(echo "$VALID_CONIDS" | wc -w)
                    ...
                    INVALID_CONIDS="$INVALID_CONIDS $CONID"
                fi
            else
                INVALID_OPTIONS=$((INVALID_OPTIONS + 1))
                INVALID_CONIDS="$INVALID_CONIDS $CONID"
...
if [ $TOTAL_TESTS -gt 0 ]; then
    SUCCESS_RATE=$(( (VALID_OPTIONS * 100) / TOTAL_TESTS ))
    echo -e "  Erfolgsrate:             ${SUCCESS_RATE}%"
else
    echo -e "  Erfolgsrate:             0%"
fi
...
# Anzahl valider CONIDs
VALID_CONID_COUNT=$(echo "$VALID_CONIDS" | wc -w)
if [ "$VALID_CONID_COUNT" -gt 0 ]; then
    print_result "VALIDE CONIDs ($VALID_CONID_COUNT)"
    echo -e "${CYAN}  ┌────────────────────────────────────────────────────────────┐${NC}"
    ...
    for CONID in $VALID_CONIDS; do
        ...
        # Formatierte Ausgabe
        printf "${CYAN}  │ %-3d. %-10s │ %-25s │${NC}\n" $CONID_INDEX "$CONID" "$DESCRIPTION"
        printf "${CYAN}  │     Bid: %-6s Ask: %-6s Spread: %-6s │${NC}\n" "$BID" "$ASK" "$SPREAD"
        
        if [ $CONID_INDEX -lt $VALID_CONID_COUNT ]; then
            echo -e "${CYAN}  ├────────────────────────────────────────────────────────────┤${NC}"
        fi
        
        CONID_INDEX=$((CONID_INDEX + 1))
        IFS=' '
    done
    IFS=$OLD_IFS
...
if [ "$VALID_CONID_COUNT" -gt 0 ]; then
    ...
    for CONID in $VALID_CONIDS; do
        ...
        if echo "$DESCRIPTION" | grep -qE " C | CALL"; then
            CALL_COUNT=$((CALL_COUNT + 1))
        elif echo "$DESCRIPTION" | grep -qE " P | PUT"; then
    ...
    IFS=' '
    for CONID in $VALID_CONIDS; do
        DESC=$(echo "$RESPONSE_BODY" | jq -r ".[] | select(.conid == $CONID) | .description" 2>/dev/null)
        EXP=$(echo "$DESC" | grep -oE "[0-9]{2}[A-Z]{3}[0-9]{2}|[0-9]{4}-[0-9]{2}-[0-9]{2}" | head -1)
        if [ -n "$EXP" ]; then
    ...
    EXPIRATIONS=$(echo "$EXPIRATIONS" | sort | uniq | head -5)
    
    if [ -n "$EXPIRATIONS" ]; then
        echo -e "  Expirations (erste 5):"
        echo "$EXPIRATIONS" | while read -r exp; do
            echo -e "    - $exp"
        done
    fi
fi
